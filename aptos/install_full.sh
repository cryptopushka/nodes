...
sudo chmod a+x /usr/local/bin/yq &>/dev/null
cd $HOME
...
then
    ...
    sudo apt-get install ca-certificates curl gnupg lsb-release wget -y &>/dev/null
...
docker pull aptoslab/validator:devnet &>/dev/null
docker pull aptoslab/tools:devnet &>/dev/null
rm *
rm $HOME/aptos/identity/id.json
wget -P $HOME/aptos https://raw.githubusercontent.com/aptos-labs/aptos-core/main/docker/compose/public_full_node/docker-compose.yaml &>/dev/null
